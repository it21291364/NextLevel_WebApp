...
const UpdateProduct = ({ match, history }) => {
    ...
    useEffect(() => {
        ...
        if (product && product._id !== productId) {//checks if the product object exists and id property is not equal to productId
            dispatch(getProductDetails(productId));//true-->retrieve the product details from the backend API.
        } else {// if the product object exists 
    ...
    const onChange = e => {
        ...
        files.forEach(file => {
            ...
            reader.onload = () => {//load event is fired when the file has finished loading.
                if (reader.readyState === 2) {//2==>file has been completely loaded.
