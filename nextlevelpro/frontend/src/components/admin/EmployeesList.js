...
const EmployeesList = ({ history }) => {
    ...
    useEffect(() => {
        ...
        if (error) {//if there is an error in the "employees" state 
            alert.error(error);
        ...
        if (isDeleted) {//checks if a employee has been successfully deleted
    ...
    const setEmployees = () => {//creates a table of employees data
        const data = {
        ...
        employees.forEach(employee => {
            data.rows.push({//creates an object with properties --->columns
                ...
                actions: <Fragment>
                    ...
                    <button className="btn btn-danger py-1 px-2 ml-2" onClick={() => deleteEmployeeHandler(employee._id)}>
    ...
    const deleteEmployeeHandler = (id) => {
        ...
        const generatePDF = () => {
            ...
            // Add table headers
            const headers = ['ID', 'Name', 'Position', 'Salary', 'Phone Number', 'Email'];
            tableRows.push(headers);
        
            // Add table data
            employees.forEach(employee => {
                const employeeData = [employee._id, employee.name, employee.position, `Rs${employee.salary}`, employee.phoneNumber, employee.email];
