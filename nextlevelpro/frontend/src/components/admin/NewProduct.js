...
const NewProduct = ({ history }) => {//history--->previously type things on form
    ...
    const submitHandler = (e) => {//executed when the user submits the new product form
    ...
    const onChange = e => {//when the user selects one or more files in an input field.----->images
        ...
        files.forEach(file => {
            ...
            reader.onload = () => {//called when the file has been loaded into memory.
                if (reader.readyState === 2) {//indicating that the file has been completely loaded
