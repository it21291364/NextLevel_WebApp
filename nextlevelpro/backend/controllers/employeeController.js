...
exports.newEmployee = catchAsyncErrors(async (req, res, next) => {//handles the creation of a new employee
...
exports.getAdminEmployees = catchAsyncErrors(async (req, res, next) => {//admin can get all employees details
...
exports.deleteEmployee = catchAsyncErrors(async (req, res, next) => {//handle HTTP DELETE requests to delete a employee from the database

    const employee = await Employee.findById(req.params.id);//find the employee in the database with the specified ID 

    if (!employee) {//If the employee is not found, this line creates and throws a new ErrorHandler
...
exports.updateEmployee = catchAsyncErrors(async (req, res, next) => {
    ...
    if (!employee) {//checks if the employee variable is existing
        return next(new ErrorHandler('Employee not found', 404));
    }

    
    employee = await Employee.findByIdAndUpdate(req.params.id, req.body, {//update the employee in the database by calling the id
