...
exports.newProduct = catchAsyncErrors(async (req, res, next) => { //handles the creation of a new product

    let images = [] //Initializes an empty images array.
    if (typeof req.body.images === 'string') { //Checks if the images field in the request body is a string
    ...
    for (let i = 0; i < images.length; i++) { //Loops through each image in the images array.
        const result = await cloudinary.v2.uploader.upload(images[i], { //uploads it to the Cloudinary service
            folder: 'products'
        });
    ...
    res.status(201).json({//Sends a response to the client 
        success: true,
        product
    })
})
...
exports.getAdminProducts = catchAsyncErrors(async (req, res, next) => { //admin can get all products details 
...
exports.getSingleProduct = catchAsyncErrors(async (req, res, next) => {
    ...
    if (!product) {//checks if the product variable is existing
...
exports.updateProduct = catchAsyncErrors(async (req, res, next) => {
    ...
    let images = []
    if (typeof req.body.images === 'string') {// object is a string or an array.
    ...
    if (images !== undefined) {//checks if the images variable is not undefined
    ...
    product = await Product.findByIdAndUpdate(req.params.id, req.body, {//updates the product in the database by calling the id
...
exports.deleteProduct = catchAsyncErrors(async (req, res, next) => {//handle HTTP DELETE requests to delete a product from the database

    const product = await Product.findById(req.params.id);//ind the product in the database with the specified ID 

    if (!product) {//If the product is not found, this line creates and throws a new ErrorHandler
        return next(new ErrorHandler('Product not found', 404));
    }
...
exports.createProductReview = catchAsyncErrors(async (req, res, next) => {//handle HTTP POST requests to create a review for a product

    const { rating, comment, productId } = req.body;//destructures the rating, comment, and productId properties

    const review = {//creates a new review object, which will be added to the reviews array of the product.
        user: req.user._id,
        name: req.user.name,
    ...
    if (isReviewed) {//If the user has already reviewed the product,updates their existing review with the new rating and comment.
    ...
    } else {//adds the new review to the reviews array and updates the numOfReviews property
...
exports.getProductReviews = catchAsyncErrors(async (req, res, next) => {
    ...
    res.status(200).json({//review was created successfully.
        success: true,
...
// defines a function to delete a review of a product and exports it from the module.
    ...
    await Product.findByIdAndUpdate(req.query.productId, {//updates the product with the new review data
        reviews,
        ratings,
        numOfReviews
    }, {
    ...
    res.status(200).json({//sends a success response to the client
        success: true
    })
})